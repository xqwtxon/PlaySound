    ...
    public function execute(CommandSender $sender, string $label, array $args): void
    {
        ...
        if ($sender instanceof Player) {
            ...
            if (!isset($args[1])) {
                ...
                if ($target === null) {
                    $sender->sendMessage(TextFormat::RED . "Sorry, we couldnt find player: " . $args[1]);
                    return;
                } else {
                    if ($args[0] === "all") {
                        $this->stopSound($target);
                        $sender->sendMessage(TextFormat::GREEN . "Stopped all sound to " . $args[1]);
                        return;
                    } else {
                        $this->stopSound($target, $args[0]);
                        $sender->sendMessage(TextFormat::GREEN . "Stopped ". $args[0] ." sound to " . $args[1]);
                        return;
                    }
                }
            }
            ...
            if (!isset($args[0])) {
                ...
                $target = $this->getOwningPlugin()->getServer()->getPlayerExact($args[0]);
                if ($target === null) {
                    $sender->sendMessage(TextFormat::RED . "Sorry, we couldnt find player: " . $args[0]);
                    return;
                } else {
                    if ($args[1] === "all") {
                        $this->stopSound($target);
                        $sender->sendMessage(TextFormat::GREEN . "Stopped all sound to " . $args[0]);
                        return;
                    } else {
                        $this->stopSound($target, $args[1]);
                        $sender->sendMessage(TextFormat::GREEN . "Stopped ". $args[0] ." sound to " . $args[0]);
                        return;
                    }
                }
            }
